...
excecao(utente(11,mara,34,morada('Rua do Pinheiro','Sao Lourenco','Guimaraes'))).
...
+prestador( O,B,C,D ) :: ( solucoes( N,( prestador( 7,N,X,T ),nao( nuloI( X ) ) ),L ),
					comprimento( L,Aux ),
					Aux == 0
					).
...
-cuidado(1,data( 2,1,2018,20 ),1,5,'curativo',10,'Hospital de Braga' ).
...
cuidado( 2,data( 2,1,2018,20 ),2,6,'investigacao',xpto444,'Hospital Privado de Braga' ).
cuidado( 2,data( 2,1,2018,20 ),2,6,'investigacao',xpto444,'Hospital de Braga' ).
nuloI( xpto444 ).
excecao( cuidado( 2,data( 2,1,2018,20 ),2,6,'investigacao',xpto444,'Hospital Privado de Braga' ) ).
excecao( cuidado( 2,data( 2,1,2018,20 ),2,6,'investigacao',xpto444,'Hospital de Braga') ).
...
excecao( cuidado( 5,data( 3,3,2018,20 ),5,9,'rotina',25,'Hospital Privado de Braga' ) ).
excecao( cuidado( 5,data( 3,3,2018,20 ),5,9,'exame',25,'Hospital Privado de Braga' ) ).
...
excecao( cuidado( 7,data(4,4,2018,20),7,1,'exame',69,'Hospital de Guimaraes' ) ).
excecao( cuidado( 7,data(5,4,2018,20),7,1,'exame',69,'Hospital de Guimaraes' ) ).


excecao( cuidado( 8,data(1,1,2018,20),8,2,'cirurgia',Custo,'Hospital Privado de Guimaraes' ) ) :-
	Custo >= 100,
	Custo =< 500.
...
nuloD(xpto111).
nuloD(xpto222).
cuidado(9,data(28,5,2018,20),0,3,xpto111,xpto222,'Hospital de Guimares').
...
cuidado( 13,data( 1,7,2018,20 ),13,12,'rotina',10,'Hospital do Porto' ).

cuidado( 14,data( 4,7,2018,20 ),14,11,xpto112,xpto999,'Hospital do Porto' ).
...
+cuidado( _,_,IdU,_,_,_,_ ) :: (solucoes( IdU,demonstrador( utente( IdU,A,_,_ )+excecao( utente( IdU,_,_,_ ) ),verdadeiro ),L ),
						comprimento( L,X ),
						X >= 1).

+cuidado( _,_,_,IdP,_,_,_ ) :: (solucoes( IdP,demonstrador( prestador( IdP,A,_,_ )+excecao( prestador( IdP,_,_,_ ) ),verdadeiro ),L ),
						comprimento( L,X ),
						X >= 1).
...
+( -utente( Id,No,I,M ) ) :: (solucoes( A,-utente( Id,No,I,M ),L ),
									comprimento( L,N ),
									N =< 2). % ----- aqui é menor ou igual a 2 pois ele encontra sempre o do fecho transitivo 
...
-utente( A,N,I,M ) :: ( solucoes( D,( excecao( cuidado( ID,Data,A,P,D,C,In ) ) ),L ),
						comprimento( L,Aux ),
						Aux == 0
						).
...
% --------------- Penso que na remocao de prestadores e utentes temos de ter atencao em eles estarem também em cuidados mas n se saber se sao mm eles------------------------
-prestador( A,N,I,M ) :: ( solucoes( D,( excecao( cuidado( ID,Data,U,A,D,C,In ) ) ),L ),
						comprimento( L,Aux ),
						Aux == 0
						).
...
%------------------------------------------------------------
% Extensao do predicado comparaData: Criterio(>=,<=), Data, Data -> {V,F}
